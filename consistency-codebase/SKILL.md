...
- If an SSOT kanban, explicit task plan, or ordered backlog is active and still has open in-scope tasks, `DONE` means only that the consistency/documentation substep is finished.
- Never treat documentation normalization as permission to stop the broader implementation loop when other in-scope tasks remain.
- After the markdown or workflow edit, return control to the next required task such as code changes, tests, verification, commit, or the next board item.
- Use this skill to clarify and unblock the broader flow, not to replace the broader flow.

## Next-Step Handoff Rule

- Before closing the skill, look for the next concrete task in the active flow.
- Prefer naming the most likely next step explicitly, such as implementing the documented change, adding tests, validating the workflow, or moving to the next backlog item.
- If several tasks remain, suggest the nearest unblocked one instead of stopping at "docs updated".
