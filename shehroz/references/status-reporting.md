...
## Project Progress Triad

When the operator asks for project progress or asks whether work is really
moving, inspect these three surfaces together:

1. WezTerm panes
   - visible worker pane text
   - manager or observer pane when relevant
   - timed re-check when `active` might only mean an open prompt
2. Trello board
   - current lane of the active card
   - nearby blocked/review context
   - whether the board is ahead of or behind live execution
3. Related JSONL or project-run artifacts
   - today's `~/.codex/sessions/.../rollout-*.jsonl`
   - project run-root JSONL/heartbeat files when they exist
   - file growth, last timestamp, and terminal events

Do not classify a project as `not idle` from one surface alone. If the panes,
Trello board, and JSONL disagree, say which surface is stale and which one is
the most trustworthy now.
